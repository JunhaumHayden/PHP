...
try {
    $query = $connection->query("SELECT * FROM tb_alunos");
    $produtos = $query->fetchAll(PDO::FETCH_ASSOC);

    echo "<table border='1'>";
    echo "<tr><th>matricula</th><th>Nome</th><th>media</th></tr>";
    foreach ($produtos as $produto) {
        echo "<tr><td>{$produto['matricula']}</td><td>{$produto['nome']}</td><td>{$produto['media']}</td></tr>";
    }
    echo "</table>";
